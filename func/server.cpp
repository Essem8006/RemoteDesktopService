...
int main() {
    ...
    while (true) {
        ...
        if (send(clientSocket, &frame.height, sizeof(int), 0)  < 0) {
            perror("Send failed on height");
        }
        if (send(clientSocket, &frame.width, sizeof(int), 0)  < 0) {
            perror("Send failed on width");
        }

        int checkwidth;
        int bytesReceived = recv(clientSocket, &checkwidth, 4, 0);
        if (bytesReceived > 0) {
            if (checkwidth != frame.width) {
                perror("Client not synced");
                break;
            }
        }
